...
select deptno, avg(nvl(sal,0)) "avg" from emp group by deptno, job order by deptno;
select max(sal+nvl(comm,0)) "max", min(sal+nvl(comm,0)) "min", round(avg(sal+nvl(comm,0)),1) "acg" from emp;
select substr(birthday,4,2) bir, count(*) from student group by substr(birthday,4,2) order by bir;
select count(*)|| 'EA' total, decode(substr(birthday,4,2),'01',0)||'EA' JAN
from student;
